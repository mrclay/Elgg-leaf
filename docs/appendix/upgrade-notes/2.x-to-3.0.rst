 ...
 * Event **login, <page>**: Use **register, <page>**: Use your own pagehandler or route hook
 * Hook **members:config, <type>**: Use the hook **send:before, annotation**
 * Event **pagesetup, annotations**: Use **delete, layout**: Use **view, layout**: Use **view_vars, menu:filter:members**, notifications**
 * Hook **output:before, page/layout/<layout_name>**
 * Hook **email, page/layout/<layout_name>**
 * Hook **output:after, system** instead
 * Hook **index, system**: Override the ``resources/index`` view
 * Hook **object:notifications, system**: Use **zend:message, system**: Use more granular **<hook>, system**: Use the menu or page shell hooks instead
 * Event **upgrade, system:email** hook
 * Hook **members:list, system:email** hooks
 * Hook **email:message, upgrade**: Use **upgrade, user**: Use **login:before** or **login:after**. Note the user is not logged in during the **login:before** event
 * Event **delete
