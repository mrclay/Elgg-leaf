...
class EntityTable {
	...
	function getRow($guid) {
		...
		return $this->db->getDataRow("SELECT * from {$this->db->getTablePrefix()}entities where guid=$guid and $access");
	}
	...
	function exists($guid) {
		...
		$query = "SELECT count(*) as total FROM {$this->db->getTablePrefix()}entities WHERE guid = $guid";
