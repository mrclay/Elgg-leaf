...
// e.g. viewtype page shells set content-type headers
	...
	public function respondWithError($error, $status_code = ELGG_HTTP_BAD_REQUEST, array $headers = []) {
		...
		if (!$this->isAction()) {
			...
			// Serve error page for 400, 403, and 404 error codes
	...
	public function redirect($forward_url = REFERRER, $status_code = ELGG_HTTP_FOUND) {
		...
		$forward_url = $this->hooks->trigger('forward', (string) $status_code, $params, $forward_url);
		...
		if ($status_code == 'walled_garden' || $status_code = 'csrf') {
		...
		if ($this->isXhr()) {
			...
			$output = ob_get_clean();
			return $this->respond(new OkResponse($output, $status_code, $forward_url));
		} else {
